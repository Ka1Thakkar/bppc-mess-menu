...
export default function Home() {
  ...
  return (
    <main className=' bg-black min-h-screen relative overflow-x-hidden'>
      ...
      <div className='pb-10'>
        <h1 className='text-4xl font-medium text-gray-300'>{date}</h1>
      </div>
      <div className={mont.className + ' flex flex-col lg:grid lg:grid-cols-3 w-full gap-10 text-black pb-20'}>
        ...
        <h1 className='lg:text-5xl text-4xl pb-5 font-extrabold'>Breakfast</h1>
          ...
          <div className='flex flex-col gap-2 text-base lg:text-xl font-medium text-center text-textgreen'>{data[selected_day].B.map((menuitem, key) => (
        ...
        <div className=' flex flex-col justify-start items-center p-10 border-4 border-textgreen text-white rounded-xl'>
          ...
          <div className='flex flex-col gap-2 text-base md:text-xl font-medium text-center px-10 text-textgreen'>{data[selected_day].L.map((menuitem, key) => (
            <h1 key={key}>{menuitem}</h1>
          ))}</div>
        </div>
        <div className=' flex flex-col justify-start items-center p-10 border-4 border-textgreen text-white rounded-xl'>
          <h1 className='text-4xl md:text-5xl pb-5 font-extrabold'>Dinner</h1>
          <div className='flex flex-col gap-2 text-base md:text-xl font-medium text-center px-10 text-textgreen'>{data[selected_day].D.map((menuitem, key) => (
            <h1 key={key}>{menuitem}</h1>
          ))}</div>
        </div>
      </div>
